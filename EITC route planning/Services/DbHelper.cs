...

    {
                ...
                using (SqlDataReader reader = commandSections.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        CachedSection cachedSection =
                            new CachedSection(from, to, price, duration, weight, category, provider);
        ...
        public static void SaveCachedSections(List<CachedSection> cachedSections)
        {
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                SqlCommand command =
                    new SqlCommand(
                        "INSERT INTO dbo.CachedSection(price, duration, fromCity, toCity, provider, weight, category_name) VALUES(@Price, @Duration, UPPER(@FromCity), UPPER(@ToCity), @Provider, @Weight, @Category)",
                        conn);
